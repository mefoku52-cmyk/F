...
def generate(result: Dict[str, Any], output_path: str) -> None:
    lines: List[str] = []
    lines.append(f"# ForensicSuite Report v{result.get('version', '1.0.0')} – {result['project_path']}")
    lines.append("")
    ...
    for plugin_name, plugin_result in result.get("plugins", {}).items():
        ...
        if plugin_result["status"] != "ok":
            lines.append(f"⚠️ Zlyhal: {plugin_result.get('error', plugin_result.get('reason', 'unknown'))}")
            lines.append("")
            continue
        ...
        if plugin_name == "filesystem":
            ...
            lines.append(f"- Nepotvrdené zmeny: {'áno' if data.get('uncommitted_changes') else 'nie'}")
        lines.append("")
