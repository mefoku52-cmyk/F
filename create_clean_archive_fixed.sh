...
# 2. Vytvor dočasný adresár v HOME (nie v /tmp)
TEMP_DIR="$HOME/tmp/${ARCHIVE_NAME}"
mkdir -p "$TEMP_DIR"
...
if [ -d "$DOWNLOAD_DIR" ]; then
    cp "$HOME/tmp/${ARCHIVE_NAME}.tar.gz" "$DOWNLOAD_DIR/"
    ...
    cp "$HOME/tmp/${ARCHIVE_NAME}.zip" "$DOWNLOAD_DIR/"
    echo "✅ ZIP archív uložený do: $DOWNLOAD_DIR/${ARCHIVE_NAME}.zip"
else
    echo "⚠️  Download adresár nenájdený, ukladám lokálne..."
    cp "$HOME/tmp/${ARCHIVE_NAME}.tar.gz" "$HOME/"
fi

# 8. Vyčisti dočasné súbory
rm -rf "$TEMP_DIR" 2>/dev/null
rm -f "$HOME/tmp/${ARCHIVE_NAME}.tar.gz" 2>/dev/null
rm -f "$HOME/tmp/${ARCHIVE_NAME}.zip" 2>/dev/null
...
# 9. Zobraziť obsah archívu
echo ""
echo "📋 OBSAH ARCHÍVU (prvých 20 položiek):"
tar -tzf /sdcard/Download/${ARCHIVE_NAME}.tar.gz 2>/dev/null | head -20
