...
echo "=== OPRAVA DASHBOARDU – finálna verzia ==="

# 1. Vytvorenie priečinka pre logo
mkdir -p dashboard/assets

# 2. Vytvorenie HTML s logom ako pozadím
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Logo na pozadí – jemné, neprebíjajúce */
  .logo-bg {
    position: fixed;
    top: 50%;
    left: 50%;
    ...
    letter-spacing: 8px;
    line-height: 1.2;
    text-shadow: 0 0 60px rgba(96,165,250,0.1);
    user-select: none;
    white-space: nowrap;
  }
  .logo-bg small {
  ...
  @media (max-width: 768px) {
    ...
    .logo-bg small { font-size: 20px; letter-spacing: 6px; }
  }
</style>
</head>
<body>
...
function render(data) {
  ...
  // Radarový graf – opravené, aby sa inicializoval správne
...
echo "[OK] dashboard/index.html – opravené, pridané logo a pozadie"

# 2. Vytvorenie core/fixer.py (ak neexistuje)
...
# 3. Spustenie API servera (ak nebeží)
echo ""
...
echo "  - Tvoje logo ako jemné pozadie"
echo "  - Ozubené kolesá (jemné)"
echo "  - Radarový graf – opravený"
echo "  - Všetky tlačidlá funkčné"
echo "  - História a trend"
echo ""
echo "Ak PDF export stále nefunguje, skontroluj API server:"
echo "  curl http://localhost:8765/health"
echo "  curl http://localhost:8765/export/pdf -X POST"
