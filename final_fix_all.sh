...
echo "=== FINÁLNA OPRAVA – VŠETKO NARAZ ==="

# 1. Zastavíme staré procesy
pkill -f "http.server" 2>/dev/null || true
pkill -f "server.api" 2>/dev/null || true
sleep 1

# 2. Vytvoríme nový server/api.py – automatický sken pri štarte
...
echo "[OK] server/api.py – automatický sken pri štarte"

# 3. Upravíme dashboard/index.html – logo viditeľnejšie, fallback pre Chart.js
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Logo na pozadí – zlaté, celé meno, viditeľné */
  .logo-bg {
    position: fixed;
    top: 50%;
    left: 50%;
  ...
  @media (max-width: 768px) {
    ...
    .logo-bg { font-size: 60px; white-space: normal; }
    .logo-bg small { font-size: 22px; letter-spacing: 8px; }
  }
</style>
</head>
<body>

<!-- Logo na pozadí – zlaté, celé meno -->
<div class="logo-bg">
  RADOSLAV
...
// --- Načítanie report.json alebo last_report ---
async function loadReport() {
  // Najprv skúsime načítať lokálny report.json
  try {
  ...
  // Ak nie je lokálny, skúsime API last_report
  try {
    const result = await callApi('/last_report');
  ...
  // Ak nič nefunguje, ukážeme dropzone
  loadingEl.style.display = 'none';
...
function render(data) {
  ...
  if (radarData.length > 0 && typeof Chart !== 'undefined') {
    try {
      ...
      radarCtx.parentElement.innerHTML = '<div style="color:#f87171;padding:20px;">Chyba radaru: ' + e.message + '</div>';
    }
  } else if (radarData.length > 0) {
    radarCtx.parentElement.innerHTML = '<div style="color:#94a3b8;padding:20px;">Radar nie je dostupný (Chart.js chýba).</div>';
  }
  ...
  if (sevCtx && typeof Chart !== 'undefined') {
    ...
    try {
      ...
      sevCtx.parentElement.innerHTML = '<div style="color:#f87171;padding:20px;">Chyba grafu: ' + e.message + '</div>';
    }
  }
...
function renderTrendFromHistory(history) {
  ...
  try {
    ...
    trendCtx.parentElement.innerHTML = '<div style="color:#f87171;padding:20px;">Chyba trendu: ' + e.message + '</div>';
  }
}
...
echo "[OK] dashboard/index.html – logo viditeľnejšie, fallback pre Chart.js"

# 4. Spustenie API servera a HTTP servera
echo ""
echo "=== SPÚŠŤAM SERVERY ==="
...
cd ..
sleep 2
echo "✅ HTTP server spustený (port 8080)"

echo ""
echo "=== HOTOVO ==="
echo "Teraz otvor: http://localhost:8080"
echo "Ak sa radar stále nezobrazuje, skontroluj konzolu prehliadača (F12 → Console)."
